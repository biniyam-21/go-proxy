...
var backendQueue chan *Backend //  Queue of available backends
var requestBytes map[string]int64
var requestLock sync.Mutex //  protects access to requestBytes

func init() {
	requestBytes = make(map[string]int64)
	backendQueue = make(chan *Backend, 10) //   Start with a small queue so we can drain it before accepting new connections
}
